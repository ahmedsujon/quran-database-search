<div class="query-search-page">
    <style>
        ...
        .query-search-page .query-list-item:hover .query-list-chevron {
            color: var(--query-accent-hover);
    ...
    <div class="container mt-4" style="margin-top: 70px !important;">
        <div class="row mb-3 align-items-start">
            <div class="col-12 col-lg-8 mb-3 mb-lg-0">
                <h4 class="mb-1 text-center text-lg-start fw-semibold">Choose a canned query</h4>
                @if (session('menu_name'))
                    <p class="text-muted small mb-0 text-center text-lg-start">{{ session('menu_name') }}</p>
                @endif
            </div>
            <div class="col-12 col-lg-4 text-center text-lg-end">
                <a href="{{ route('app.ConductSearch') }}" class="btn btn-outline-secondary btn-sm me-2 mb-2 mb-sm-0">
                    Main menu
                </a>
            ...
            <div class="breadcrumb-card px-4 py-3">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item">
                        <a href="{{ route('app.ConductSearch') }}" class="text-decoration-none">
        ...
        <div class="card query-list-card border-0">
            <div class="card-header bg-white py-3 d-flex flex-wrap align-items-center justify-content-between gap-2">
                <span class="fw-semibold text-dark">Name of query</span>
                <span class="badge rounded-pill bg-light text-secondary border">
                    {{ $searchValues->count() }} {{ $searchValues->count() === 1 ? 'query' : 'queries' }}
                </span>
            </div>
            @if ($searchValues->isEmpty())
                <div class="card-body empty-state mb-0">
                    No queries are available for this menu yet.
                </div>
            @else
                <div class="list-group list-group-flush">
                    @foreach ($searchValues as $item)
                        <a
                            ...
                            class="list-group-item list-group-item-action query-list-item border-start-0 border-end-0"
                            role="button"
                        >
                            <span class="query-list-index">{{ $loop->iteration }}</span>
                            <span class="query-list-text">{{ $item->topic }}</span>
