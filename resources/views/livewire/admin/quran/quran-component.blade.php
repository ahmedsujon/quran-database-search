            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0 font-size-18">Quran Data</h4>
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                                <li class="breadcrumb-item active">Quran Data</li>
                            </ol>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header bg-white" style="border-bottom: 1px solid #e2e2e7;">
                            <h4 class="card-title" style="float: left;">All Quran Data</h4>
                        </div>
                        <div class="card-body">
                            <div class="row mb-3">
                            ...
                            <div class="table-responsive">
                                <table class="table align-middle table-nowrap table-bordered mb-0">
                                    ...
                                    <tbody>
                                        @if ($quran_datas->count() > 0)
                                            @php
                                                $sl =
                                                    $quran_datas->perPage() * $quran_datas->currentPage() -
                                                    ($quran_datas->perPage() - 1);
                                            @endphp
                                            @foreach ($quran_datas as $quran_data)
                                                <tr>
                                                    <td>
                                                        {{ $quran_data->surah_no }}:{{ $quran_data->ayat_no }}</td>
                                                    <td>{{ $quran_data->quran_english }}</td>
                                                    <td>{{ $quran_data->quran_arabic }}</td>
                                                </tr>
                                            @endforeach
                                        @else
                                            <tr>
                        ...
                        <div class="card-footer bg-transparent">
                            {{ $quran_datas->links('livewire.admin-pagination') }}
                        </div>
                    </div>
                </div>
            </div>
